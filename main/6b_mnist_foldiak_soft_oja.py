...
if os.path.exists(results_path):
    ...
    if not all("digit_selectivity" in r for r in results):
        results=[]
else:
    results=[]
if len(results)==0:
    for resolution in resolutions:
        ...
        for number_of_outputs in output_sizes:
            qij,wij,ti,activity=funs.train_soft_oja_foldiak(x_train[:num_batches], number_of_outputs, p, alpha, beta, gamma, lambda_, dt, settling_steps, num_batches, pretrain_patterns)
            z_train=funs.encode_soft_foldiak(x_train, qij, wij, ti, lambda_, dt, settling_steps)
            z_test=funs.encode_soft_foldiak(x_test, qij, wij, ti, lambda_, dt, settling_steps)
            sparse_train_acc,sparse_test_acc=funs.perceptron_accuracy(z_train, train_y.numpy(), z_test, test_y.numpy())
            layer2_output_size=number_of_outputs
            q2,w2,t2,activity2=funs.train_soft_oja_foldiak(z_train[:num_batches], layer2_output_size, p2, alpha, beta, gamma, lambda_, dt, settling_steps, num_batches, pretrain_patterns)
            z2_train=funs.encode_soft_foldiak(z_train, q2, w2, t2, lambda_, dt, settling_steps)
            z2_test=funs.encode_soft_foldiak(z_test, q2, w2, t2, lambda_, dt, settling_steps)
            ...
            result={"resolution":resolution, "number_of_outputs":number_of_outputs, "layer2_output_size":layer2_output_size, "raw_test_acc":raw_test_acc, "sparse_test_acc":sparse_test_acc, "layer2_test_acc":layer2_test_acc, "both_test_acc":both_test_acc, "sparsity":np.mean(z_train), "layer2_sparsity":np.mean(z2_train), "filter_similarity":funs.mean_abs_offdiag_cosine(qij), "sta_similarity":funs.mean_abs_offdiag_cosine(rf1_sta), "digit_selectivity":funs.digit_selectivity(z_train,train_y.numpy()), "layer2_digit_selectivity":funs.digit_selectivity(z2_train,train_y.numpy()), "rf_top10_energy":funs.top_energy_fraction(rf1_sta), "layer2_rf_top10_energy":funs.top_energy_fraction(rf2_sta), "activity":activity, "activity2":activity2, "qij":qij, "q2":q2, "rf1_sta":rf1_sta, "rf2_sta":rf2_sta}
...
ax.bar(x-0.30, raw_acc, width=0.20, color="0.55", edgecolor="white", linewidth=.8, alpha=.82, label="raw pixels")
ax.bar(x-0.10, sparse_acc, width=0.20, color="#4f6fd5", edgecolor="white", linewidth=.8, alpha=.88, label="layer 1")
ax.bar(x+0.10, layer2_acc, width=0.20, color="#d62728", edgecolor="white", linewidth=.8, alpha=.78, label="layer 2")
ax.bar(x+0.30, both_acc, width=0.20, color="#2ca02c", edgecolor="white", linewidth=.8, alpha=.82, label="layer 1+2")
ax.set_xticks(x)
ax.set_xticklabels(names, rotation=20, ha="right")
ax.set_ylim(0,1)
ax.set_ylabel("test accuracy")
ax.set_title("single-layer perceptron on raw MNIST vs soft Oja codes")
...
#%%
fig,ax=plt.subplots(figsize=(8.5,3.2),dpi=220,facecolor="white")
ax.bar(x-0.16, sparsity, width=0.32, color="#4f6fd5", edgecolor="white", linewidth=.8, alpha=.86, label="layer 1")
ax.bar(x+0.16, layer2_sparsity, width=0.32, color="#d62728", edgecolor="white", linewidth=.8, alpha=.78, label="layer 2")
ax.axhline(p, color="0.45", lw=.9, ls="--", label="target p")
ax.axhline(p2, color="#d62728", lw=.9, ls=":", label="target p2")
ax.set_xticks(x)
ax.set_xticklabels(names, rotation=20, ha="right")
ax.set_ylabel("mean activity")
ax.set_title("mean activity of learned MNIST representations")
fig.legend(*ax.get_legend_handles_labels(), frameon=False, loc="upper center", bbox_to_anchor=(0.5,.98), ncol=4)
ax.spines[["top","right"]].set_visible(False)
plt.tight_layout(rect=[0,0,1,.88])
...
ax.axhline(p, color="0.45", lw=.9, ls="--", label="target p")
ax.axhline(p2, color="#d62728", lw=.9, ls=":", label="target p2")
ax.plot(x, sparsity, "-o", color="#4f6fd5", lw=1.8, ms=4.8, label="layer 1")
ax.plot(x, layer2_sparsity, "-o", color="#d62728", lw=1.8, ms=4.8, label="layer 2")
ax.set_xticks(x)
ax.set_xticklabels(names, rotation=20, ha="right")
...
ax=axes[0,0]
ax.plot(x, rf_top10_energy, "-o", color="#4f6fd5", lw=1.6, ms=3.8, label="L1")
ax.plot(x, layer2_rf_top10_energy, "-o", color="#d62728", lw=1.6, ms=3.8, label="L2")
...
ax.plot(x, sparsity, "-o", color="#4f6fd5", lw=1.6, ms=3.8, label="L1")
ax.plot(x, layer2_sparsity, "-o", color="#d62728", lw=1.6, ms=3.8, label="L2")
ax.set_title("sparsity",fontsize=10)
ax.set_ylabel("mean activity")
ax.legend(frameon=False,fontsize=7,loc="lower left")
ax=axes[1,0]
ax.plot(x, filter_similarity, "-o", color="0.35", lw=1.6, ms=3.8, label="weights")
ax.plot(x, sta_similarity, "-o", color="#9467bd", lw=1.6, ms=3.8, label="STA")
ax.set_title("feature redundancy",fontsize=10)
ax.set_ylabel("mean |cosine|")
ax.legend(frameon=False,fontsize=7,loc="best")
ax=axes[1,1]
ax.plot(x, digit_selectivity, "-o", color="#4f6fd5", lw=1.6, ms=3.8, label="L1")
ax.plot(x, layer2_digit_selectivity, "-o", color="#d62728", lw=1.6, ms=3.8, label="L2")
...
for ax in axes.reshape(-1):
    ...
    ax.spines[["top","right"]].set_visible(False)
fig.suptitle("MNIST soft Oja code summary",fontsize=12,y=.99)
...
for r in results:
    ...
    fig.suptitle(f"MNIST soft Oja activity-triggered RFs | {resolution}px | L1={number_of_outputs}, L2={layer2_output_size}",fontsize=12,y=.99)
    plt.tight_layout()
    ...
    for i,ax in enumerate(axes.reshape(-1)):
        ...
        if i<n_show1:
            ...
            ax.set_title(f"L1-{i}",fontsize=7,pad=2)
    fig.suptitle(f"MNIST soft Oja layer 1 weights | {resolution}px | L1={number_of_outputs}",fontsize=12,y=.99)
    plt.tight_layout()
