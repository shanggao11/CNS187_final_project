# Yubei Chen, Sparse Manifold Transform Lib Ver 0.1  https://github.com/yubeic/Sparse-Coding/
"""
...
def ISTA_PN(I,basis,lambd,num_iter,eta=None,device=None, useMAGMA=True):
    ...
    if eta is None:
        if useMAGMA:
            ...
            L = torch.linalg.eigvalsh(torch.mm(basis, basis.t())).max()
            eta = 1./L
        else:
    ...
    Res = (torch.FloatTensor(I.size()).fill_(0)).to(device)
    ahat = (torch.FloatTensor(M,batch_size).fill_(0)).to(device)
